...
class RuGNN(nn.Module):
    def __init__(self, h_dim):
        ...
        # TODO add another embedding layer for the global node
        ...
        # TODO 每一层的关系embedding，最后通过一个线性层得到最终的关系embedding
        self.rel_embs = nn.ParameterList(
            [get_param(self.n_rel * 2 + 1, h_dim) for _ in range(self.kg_n_layer)]
        )
        # TODO 用于规则的关系embedding
        self.r_rel_emb = get_param(self.n_rel * 2 + 1, h_dim)

        # relation SE layer
        self.edge_layers = nn.ModuleList(
            [EdgeLayer(h_dim) for _ in range(self.kg_n_layer)]
        )
        # entity SE layer
        self.node_layers = nn.ModuleList(
            [NodeLayer(h_dim) for _ in range(self.kg_n_layer)]
        )
        # triple SE layer
        self.comp_layers = nn.ModuleList(
            [CompLayer(h_dim) for _ in range(self.kg_n_layer)]
        )
        ...
        # TODO 不太懂，对多层GNN的关系embedding进行线性变换
        ...
        if self.cfg.decoder == "TransE":
            ...
            self.predictor = ConvE(
                h_dim, out_channels=self.cfg.out_channel, ker_sz=self.cfg.ker_sz
            )
    ...
    def aggragate_emb(self, kg, rules, rules_mask, IM, Ht):
        ...
        rel_emb_list = []
        for edge_layer, node_layer, comp_layer, rel_emb in zip(
            self.edge_layers, self.node_layers, self.comp_layers, self.rel_embs
        ):
...
class RuleLayer(nn.Module):  # rule sequence encoding layer
    def __init__(self, h_dim):
        ...
        # rule transformer
        self.rule_transformer = Rule_Transformer(
            h_dim,
            self.rule_hidden_dim,
            self.rule_dim,
            self.rule_len,
            self.num_head,
            self.num_encoder,
    ...
    def forward(self, rules, rules_mask, IM, rel_emb):
        ...
        rules_mask = rules_mask.unsqueeze(1)  # torch.Size([rules_num, 1, 5])
        # TODO body_embs.shape=[rules_num, 3, h_dim], head_embs.shape=[rules_num, 1, h_dim], rules_mask.shape=[rules_num, 1, 5]
