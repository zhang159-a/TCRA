...
def train_step(model, data, kg, rules, rules_mask, IM, Ht, optimizer, scheduler):
    ...
    src, rel, label, rm_edges = (
        src.to(device),
        rel.to(device),
        label.to(device),
        rm_edges.to(device),
    )
    # TODO 这里是它添加的部分，用来进行规则的处理
...
def evaluate(model, kg, rules, rules_mask, IM, Ht, set_flag, record=False) -> dict:
    ...
    with torch.no_grad():
        ...
        ent_emb, rel_emb = model.aggragate_emb(
            kg, rules.to(device), rules_mask.to(device), IM.to(device), Ht
        )
        for i, data in enumerate(eval_loader):
            # filter_bias: (bs, n_ent)
            (src, rel, dst), filter_bias, mode = data
            src, rel, dst, filter_bias = (
                src.to(device),
                rel.to(device),
                dst.to(device),
                filter_bias.to(device),
            )
            # (bs, n_ent)
            ...
            ranking_low = compare_low.to(dtype=torch.float).sum(
                dim=1
            )  # include the pos one itself, no need to +1
            ...
            results["MR"] = results.get("MR", 0.0) + ranking.sum().item()
            results["MRR"] = results.get("MRR", 0.0) + (1 / ranking).sum().item()
            for k in hits_range:
                results["HITS@{}".format(k)] = (
        ...
        for k, value in metrics["head_batch"].items():
            metrics["average"][k] = (
                metrics["head_batch"][k] + metrics["tail_batch"][k]
            ) / 2

        # sort in the ranking order
        if record:
            metrics["ranking"] = torch.cat(metrics["ranking"], dim=0).tolist()
            metrics["ranking"] = sorted(
                metrics["ranking"], key=lambda x: x[3], reverse=True
            )
