...
from model_helper import evaluate, train_step
...
def format_metrics(name, h_metric, t_metric):
    msg_h = (
        name
        + " (head) - MRR: {:5.4f}, MR: {:7.2f}, H@1: {:4.3f}, H@3: {:4.3f}, H@10: {:4.3f}"
    )
    msg_t = (
        name
        + " (tail) - MRR: {:5.4f}, MR: {:7.2f}, H@1: {:4.3f}, H@3: {:4.3f}, H@10: {:4.3f}"
    )
    msg_avg = (
        name
        + " (avg) - MRR: {:5.4f}, MR: {:7.2f}, H@1: {:4.3f}, H@3: {:4.3f}, H@10: {:4.3f}"
    )
    msg_h = msg_h.format(
        h_metric["MRR"],
        h_metric["MR"],
...
@hydra.main(config_path=join("..", "config"), config_name="config", version_base="1.1")
def main(config: DictConfig):
    ...
    optimizer = torch.optim.Adam(
        filter(lambda p: p.requires_grad, model.parameters()), lr=init_lr
    )
    ...
    for epoch in range(n_epoch):
        ...
        val_metrics = evaluate(
            model, kg, ruleset.R, ruleset.R_mask, ruleset.IM, g.Ht, set_flag="valid"
        )["average"]
        if val_metrics["MRR"] > best_mrr:
            best_mrr = val_metrics["MRR"]
            save_variables = {"best_val_metrics": val_metrics}
        ...
        if improvement_flag == "*":
            test_metrics = evaluate(
                model, kg, ruleset.R, ruleset.R_mask, ruleset.IM, g.Ht, set_flag="test"
            )["average"]
    ...
    train_metrics = evaluate(
        model, kg, ruleset.R, ruleset.R_mask, ruleset.IM, g.Ht, "train"
    )
    train_msg = format_metrics(
        "Train", train_metrics["head_batch"], train_metrics["tail_batch"]
    )
    ...
    valid_metrics = evaluate(
        model, kg, ruleset.R, ruleset.R_mask, ruleset.IM, g.Ht, "valid"
    )
    valid_msg = format_metrics(
        "Valid", valid_metrics["head_batch"], valid_metrics["tail_batch"]
    )
    ...
    test_metrics = evaluate(
        model, kg, ruleset.R, ruleset.R_mask, ruleset.IM, g.Ht, "test"
    )
    test_msg = format_metrics(
        "Test", test_metrics["head_batch"], test_metrics["tail_batch"]
    )
